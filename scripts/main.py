...
def trigger_summaries(text_to_summarize, output_folder, target_lang):
    ...
    # 1. Generate English Summaries first
    print("Generating Patient Perspective...")
    pat_summary_en = run_summarization(text_to_summarize, "Patient")
    ...
    # RETURN the values for Streamlit to display
    return pat_summary_final, doc_summary_final
...
def process_pdf(input_pdf: str, target_lang: str):
    ...
    print(f"--- Starting PDF Pipeline: {input_pdf} ---")
    pages = convert_from_path(input_pdf)
    full_text = ""
    for i, page in enumerate(pages):
        text = pytesseract.image_to_string(page, lang="eng+fra+spa")
    ...
    # Return the summary results back to the caller
    return trigger_summaries(final_text, output_dir, target_lang)
...
# ---------------- MAIN ROUTER (For Terminal Use) ---------------- #

def main():
    print("--- Medical Scribe AI System ---")
    target_lang = input("Enter target language code (e.g., 'hi', 'mr', 'en'): ").strip().lower()
    if not target_lang: target_lang = "en"
